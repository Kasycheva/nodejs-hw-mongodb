...
export const loginUser = async ({ email, password }) => {
    ...
    const accessToken = jwt.sign({ userId: user._id }, getEnvVar("JWT_SECRET"), { expiresIn: "15m" });
    const refreshToken = jwt.sign({ userId: user._id }, getEnvVar("JWT_SECRET"), { expiresIn: "30d" });
...
export const refreshUser = async (refreshToken) => {
    ...
    try {
        const decoded = jwt.verify(refreshToken, getEnvVar("JWT_SECRET"));
        const session = await Session.findOne({ refreshToken });

        if (!session) throw createHttpError(401, "Session not found");

        const newAccessToken = jwt.sign({ userId: decoded.userId }, getEnvVar("JWT_SECRET"), { expiresIn: "15m" });
        const newRefreshToken = jwt.sign({ userId: decoded.userId }, getEnvVar("JWT_SECRET"), { expiresIn: "30d" });
...
export const getUserSession = async (accessToken) => {
